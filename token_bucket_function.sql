...
CREATE OR REPLACE FUNCTION take_token (key_id VARCHAR(100)) RETURNS boolean AS $$
DECLARE
    rate int4;
    rate_period int4;
    tokens int4;
...
BEGIN
    ...
    SELECT INTO rate, rate_period r.per_period, r.period_seconds FROM token_rates r WHERE r.key_id = $1;
    ...
    SELECT b.tokens, b.last_refill INTO tokens, last_refill FROM token_buckets b WHERE b.key_id = $1;
